...
class Remove extends CommandAbstract
{
    ...
    public function __construct(Discord $discord)
    {
        $meta_data = (object) [
            'name' => 'remove',
            'description' => 'Removed a new user from the following channel.'
        ];
    ...
    public function execute(Interaction $interaction)
    {
        ...
        $message = (new MessageBuilder())
            ->setContent("User <@{$memberToAdd}> Has been Successfully removed from the {$channel->name} channel");
        ...
        $commandBuilder = (new CommandBuilder())
            ...
            ->addOption(
                (new Option($this->discord))
                    ->setName('user')
                    ->setDescription('Select the User You want to remove from this channel.')
                    ->setRequired(true)
                    ->setType(Option::USER)
            )
